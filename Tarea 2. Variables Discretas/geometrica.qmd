...
prob_5 <- dgeom(5 - 1, prob = p)  # dgeom usa (k-1)
prob_5

# b) Probabilidad de necesitar más de 3 lanzamientos
prob_mas_3 <- pgeom(3 - 1, prob = p, lower.tail = FALSE)
prob_mas_3
...
# 1) Probabilidad de que el primer acierto sea en el intento 3
prob_X_eq_3 <- dgeom(3 - 1, prob = p)  # dgeom(x) = P(#fallos = x) => #fallos = k-1
prob_X_eq_3

# 2) Esperanza (número esperado de intentos)
esperanza <- 1 / p
esperanza

# 3) Probabilidad de necesitar entre 2 y 5 intentos (inclusive)
k <- 2:5
prob_2_a_5 <- sum(dgeom(k - 1, prob = p))
prob_2_a_5

# Mostrar resultados con 4 decimales
cat("P(X=3) =", round(prob_X_eq_3, 4), "\n")
cat("E[X]   =", round(esperanza, 6), "\n")
cat("P(2<=X<=5) =", round(prob_2_a_5, 6), "\n")
...
# 1) Número esperado de inspecciones
esperanza <- 1 / p

# 2) Probabilidad de que la primera defectuosa aparezca después de la pieza 100
prob_mas_100 <- (1 - p)^100

# 3) Verificación de la propiedad de falta de memoria
s <- 50
t <- 20
lhs <- (1 - p)^(s + t) / (1 - p)^s   # P(X > s + t | X > s)
rhs <- (1 - p)^t                     # P(X > t)

# Resultados
cat("Esperanza (E[X]) =", esperanza, "\n")
cat("P(X > 100) =", prob_mas_100, "\n")
cat("P(X > s+t | X > s) =", lhs, "\n")
cat("P(X > t) =", rhs, "\n")
