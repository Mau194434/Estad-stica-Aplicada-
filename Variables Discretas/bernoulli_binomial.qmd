...
# 1. Máquina produce piezas defectuosas con probabilidad 0.02
p_defectuosa <- 0.02       # probabilidad de "éxito" (pieza defectuosa)
p_no_defectuosa <- 1 - p_defectuosa

# Probabilidad de obtener una pieza no defectuosa
cat("Probabilidad de pieza no defectuosa =", p_no_defectuosa, "\n")

# Media y varianza de la distribución Bernoulli
media <- p_defectuosa
varianza <- p_defectuosa * (1 - p_defectuosa)

cat("Media =", media, "\n")
cat("Varianza =", varianza, "\n")

# ----------------------------
# 2. Si se inspeccionan 10 piezas, ¿probabilidad de 2 defectuosas?
# Modelo: Binomial (n = 10, p = 0.02)
# ----------------------------
n <- 10
k <- 2
prob_2_defectuosas <- dbinom(k, size = n, prob = p_defectuosa)

cat("Probabilidad de encontrar exactamente 2 defectuosas =", prob_2_defectuosas, "\n")

# ----------------------------
# 3. Examen de opción múltiple con 4 opciones (responde al azar)
# ----------------------------
p_acierto <- 1 / 4   # probabilidad de éxito (acertar una pregunta)
p_error <- 1 - p_acierto

# Probabilidad de acertar una pregunta
cat("\nProbabilidad de acertar una pregunta =", p_acierto, "\n")

# Media y varianza (Bernoulli)
media_examen <- p_acierto
varianza_examen <- p_acierto * (1 - p_acierto)

cat("Media (examen) =", media_examen, "\n")
cat("Varianza (examen) =", varianza_examen, "\n")

```

```{r}
# Gráfica de la distribución binomial
x <- 0:n  # posibles valores de defectuosas (0 a 10)
y <- dbinom(x, size = n, prob = p_defectuosa)

barplot(y, names.arg = x, 
        col = "skyblue", 
        main = "Distribución Binomial: Piezas defectuosas (n=10, p=0.02)",
...
prob_8_caras <- dbinom(8, size = n, prob = p)
cat("a) P(8 caras) =", prob_8_caras, "\n")

# b) Probabilidad de obtener al menos 12 caras
prob_12omas <- sum(dbinom(12:15, size = n, prob = p))
cat("b) P(al menos 12 caras) =", prob_12omas, "\n")

# c) Media y desviación estándar
media_monedas <- n * p
desv_monedas <- sqrt(n * p * (1 - p))
cat("c) Media =", media_monedas, " Desviación estándar =", desv_monedas, "\n")
```
:::
...
```{r}
# 2. MEDICAMENTO EFECTIVO AL 85%


n2 <- 20     # número de pacientes
p2 <- 0.85   # probabilidad de curarse

# a) Probabilidad de que se curen exactamente 18 pacientes
prob_18 <- dbinom(18, size = n2, prob = p2)
cat("\nMedicamento:\n")
cat("a) P(18 curados) =", prob_18, "\n")

# b) Probabilidad de que se curen entre 15 y 19 pacientes
prob_15a19 <- sum(dbinom(15:19, size = n2, prob = p2))
cat("b) P(15 a 19 curados) =", prob_15a19, "\n")

# c) Número esperado de pacientes curados
esperanza_curados <- n2 * p2
cat("c) Número esperado de curados =", esperanza_curados, "\n")
```
...
X <- rbinom(N, size = n_x, prob = p_teo)
Y <- rbinom(N, size = m_y, prob = p_teo)

# Suma de las dos variables
Z <- X + Y

# Distribución teórica esperada
Z_teo <- rbinom(N, size = n_x + m_y, prob = p_teo)

# Comparar medias y varianzas
cat("\nTeorema de suma de Binomiales:\n")
cat("Media simulada =", mean(Z), " | Media teórica =", mean(Z_teo), "\n")
cat("Varianza simulada =", var(Z), " | Varianza teórica =", var(Z_teo), "\n")

# Graficar comparación
hist(Z, breaks = 30, freq = FALSE, col = "skyblue", 
     main = "Verificación del Teorema: X + Y ~ Bin(n+m, p)",
     xlab = "Número de éxitos")
lines(density(Z_teo), col = "red", lwd = 2)
legend("topright", legend = c("Simulación", "Teórico"), 
