...
```{r}
comparar_geom_negBin <- function(r, p, n_sim = 100000) {
  # r: número de variables geométricas a sumar
  # p: probabilidad de éxito
  # n_sim: número de simulaciones
  
  # 1. Simulación de la suma de r variables geométricas
  # rgeom genera variables con soporte {0, 1, 2, ...}
  # por lo que sumaremos rgeom() + 1 para tener {1, 2, 3, ...} si se desea.
  geom_suma <- replicate(n_sim, sum(rgeom(r, prob = p)))
  
  # 2. Generar distribución binomial negativa teórica equivalente
  negbin_teo <- rnbinom(n_sim, size = r, prob = p)
  
  # 3. Comparar gráficamente
  hist(geom_suma, probability = TRUE, breaks = 50,
       col = rgb(0.2, 0.6, 1, 0.4),
       main = paste("Comparación: Suma de", r, "Geométricas vs Binomial Negativa"),
       xlab = "Número de fracasos antes de r éxitos",
       border = "white")
  
  hist(negbin_teo, probability = TRUE, breaks = 50,
       col = rgb(1, 0.3, 0.3, 0.4), add = TRUE, border = "white")
  
  legend("topright", legend = c("Suma de Geométricas (simulada)", "Binomial Negativa (teórica)"),
         col = c(rgb(0.2, 0.6, 1, 0.6), rgb(1, 0.3, 0.3, 0.6)), pch = 15)
  
  # 4. Mostrar comparación de medias y varianzas
  cat("=== COMPARACIÓN ESTADÍSTICA ===\n")
  cat("Media (Geométrica simulada):", mean(geom_suma), "\n")
  cat("Media (Binomial Negativa teórica):", mean(negbin_teo), "\n")
  cat("Varianza (Geométrica simulada):", var(geom_suma), "\n")
  cat("Varianza (Binomial Negativa teórica):", var(negbin_teo), "\n")
}


# EJEMPLO DE USO

# Compara la suma de 5 geométricas (p=0.3) con la binomial negativa equivalente
comparar_geom_negBin(r = 5, p = 0.3, n_sim = 100000)
...
# Parámetros
p <- 0.7     # Probabilidad de encestar (éxito)
r <- 8       # Número de encestes deseados
n_sim <- 1000  # Número de simulaciones

# -------------------------------
# 1. Modelo teórico (Binomial Negativa)
# -------------------------------
# X = número de fallos antes de lograr r éxitos
# E[X] = r * (1 - p) / p
...
cat("=== MODELO TEÓRICO ===\n")
cat("Esperanza teórica (número esperado de fallos):", E_teorico, "\n")
cat("Varianza teórica:", Var_teorica, "\n\n")

# -------------------------------
# 2. Simulación de 1000 sesiones
# -------------------------------
# Devuelve el número de fallos antes de lograr 'r' encestes

fallos_sim <- rnbinom(n_sim, size = r, prob = p)

# Media y varianza simuladas
E_sim <- mean(fallos_sim)
Var_sim <- var(fallos_sim)

cat("=== RESULTADOS DE LA SIMULACIÓN ===\n")
cat("Media simulada:", E_sim, "\n")
cat("Varianza simulada:", Var_sim, "\n\n")

# -------------------------------
# 3. Comparación gráfica
# -------------------------------
# Histograma de la simulación vs. PMF teórica

# Tabla de frecuencias relativas
tab <- table(fallos_sim)
x_vals <- as.numeric(names(tab))
prob_emp <- as.numeric(tab) / n_sim

# Probabilidad teórica para cada valor observado
prob_teo <- dnbinom(x_vals, size = r, prob = p)

# Gráfico comparativo
barplot(prob_emp, names.arg = x_vals,
        col = "skyblue", border = "blue",
        main = sprintf("Comparación simulación vs teoría\nBinomial Negativa (r=%d, p=%.2f)", r, p),
        xlab = "Número de tiros fallados", ylab = "Probabilidad")


points(1:length(x_vals), prob_teo, type = "b", pch = 16, col = "red")
legend("topright",
       legend = c("Simulado (empírico)", "Teórico"),
       fill = c("skyblue", NA),
       border = c("blue", NA),
       pch = c(NA, 16),
       col = c("blue", "red"))
...
cat("=== CONCLUSIÓN ===\n")
cat(sprintf("El número esperado teórico de tiros fallados antes de lograr %d encestes es %.2f.\n", r, E_teorico))
cat(sprintf("La media simulada con %d sesiones fue %.2f.\n", n_sim, E_sim))
cat("La simulación confirma que el modelo teórico de la distribución binomial negativa\nrepresenta adecuadamente el número de fallos esperados.\n")
...
# Parámetros
p <- 0.30     # probabilidad de éxito en cada prueba
r <- 4        # éxitos necesarios
costo_por_prueba <- 10000  # $ por prueba fallida

# -------------------------
# 1) Probabilidad exacta de 12 fallos (antes de 4 éxitos)
# P(X = 12)
# -------------------------
prob_12_fallos <- dnbinom(12, size = r, prob = p)
cat(sprintf("P(X = 12) = %.8f\n", prob_12_fallos))

# -------------------------
# 2) Probabilidad de necesitar más de 20 fallos
# P(X > 20) = 1 - P(X <= 20)
# -------------------------
prob_mas_20 <- 1 - pnbinom(20, size = r, prob = p)
cat(sprintf("P(X > 20) = %.8f\n", prob_mas_20))

# -------------------------
# 3) Costo esperado en pruebas fallidas
E_X <- r * (1 - p) / p
costo_esperado <- E_X * costo_por_prueba
cat(sprintf("Esperanza teórica E[X] (número esperado de fallos) = %.6f\n", E_X))
cat(sprintf("Costo esperado en fallos = $%0.2f\n", costo_esperado))


# Simular N sesiones y estimar probabilidades / costo empírico
# -------------------------
Nsim <- 10000
sim_fallos <- rnbinom(Nsim, size = r, prob = p)  # número de fallos antes de r éxitos

# Estimadores empíricos
est_prob_12_emp <- mean(sim_fallos == 12)
est_prob_mas_20_emp <- mean(sim_fallos > 20)
est_E_emp <- mean(sim_fallos)
est_costo_emp <- est_E_emp * costo_por_prueba

cat("\n--- Resultados por simulación (N =", Nsim, ") ---\n")
cat(sprintf("Estimador empírico P(X = 12) = %.6f\n", est_prob_12_emp))
cat(sprintf("Estimador empírico P(X > 20) = %.6f\n", est_prob_mas_20_emp))
cat(sprintf("Media empírica de fallos = %.6f\n", est_E_emp))
cat(sprintf("Costo empírico promedio = $%0.2f\n", est_costo_emp))


# Rango para mostrar (0 hasta, por ejemplo, 30 fallos)
k_max <- 30
k_vals <- 0:k_max
pmf_teorica <- dnbinom(k_vals, size = r, prob = p)
pmf_empirica <- sapply(k_vals, function(k) mean(sim_fallos == k))

# Gráfica comparativa
bar_positions <- barplot(pmf_empirica, names.arg = k_vals,
                         col = "lightblue", border = "blue",
                         main = "PMF: Empírico (sim) vs Teórico (NegBin)",
                         xlab = "Número de pruebas fallidas (X)",
                         ylab = "Probabilidad",
                         ylim = c(0, max(pmf_empirica, pmf_teorica) * 1.15))

points(bar_positions, pmf_teorica, type = "b", pch = 16, col = "red")
legend("topright", legend = c("Empírico (sim)", "Teórico (NegBin)"),
       fill = c("lightblue", NA), border = c("blue", NA),
       pch = c(NA, 16), col = c("blue", "red"))
