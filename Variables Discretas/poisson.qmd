...
p_4 <- dpois(4, lambda)
# 2)
p_0_30s <- dpois(0, lambda * 0.5)
# 3)
esperado_hora <- lambda * 60
# 4)
p_8_12_2min <- sum(dpois(8:12, lambda * 2))

p_4; p_0_30s; esperado_hora; p_8_12_2min
...
# 1)
p_eq_8 <- dpois(8, lambda_500)
# 2)
p_ge_10 <- 1 - ppois(9, lambda_500)
# 3) n para P(at least 1)=0.95
lambda_req <- -log(0.05)
n_req <- ceiling(lambda_req / p_prod)  # mínimo entero
# 4) si tasa = 1.2 / 100
p_prod2 <- 1.2 / 100
lambda_500_new <- p_prod2 * n
p_ge_10_new <- 1 - ppois(9, lambda_500_new)

p_eq_8; p_ge_10; n_req; lambda_500_new; p_ge_10_new
...
lambda_h <- 2.5
# 1)
p_eq3 <- dpois(3, lambda_h)
# 2)
p0_2h <- dpois(0, lambda_h * 2)
# 3)
lambda_4h <- lambda_h * 4
p_exceed_8 <- 1 - ppois(8, lambda_4h)
# 4)
cap_min <- qpois(0.9, lambda_4h)  # menor k con P(X<=k) >= 0.9

p_eq3; p0_2h; p_exceed_8; cap_min
...
# Elegimos tamaños de bloque (n) usados para la suma
block_sizes <- c(1, 5, 10, 30, 50)

par(mfrow=c(2,2))
for (n in block_sizes) {
  # generar nsims * n Poisson, organizar en matriz n x nsims
  sims <- matrix(rpois(nsims * n, lambda), nrow = n, ncol = nsims)
  sums <- colSums(sims)
  # Normalización para CLT: (S_n - n*lambda) / sqrt(n*lambda)
  z <- (sums - n * lambda) / sqrt(n * lambda)
  
  cat("n =", n, " -> mean(z)=", mean(z), " sd(z)=", sd(z), "\n")
  
  # Hist + QQ
  hist(z, breaks=40, main=paste("n=",n), xlab="z", freq=FALSE)
  curve(dnorm(x), add=TRUE)
  qqnorm(z, main=paste("QQ n=",n)); qqline(z)
}
...
# Teóricas para Poisson(6.3)
lambda_sum <- 3.5 + 2.8
mean_theo <- lambda_sum
var_theo <- lambda_sum

# Comparar medias/varianzas
cat("Empírica mean=", mean_s, " var=", var_s, "\n")
cat("Teórica mean=", mean_theo, " var=", var_theo, "\n")

# Comparar histogramas y overlay Poisson(6.3) pmf (frecuencia relativa)
hist(s, breaks=seq(-0.5, max(s)+0.5, 1), freq=FALSE, main="Suma X1+X2")
# overlay pmf
xvals <- 0:max(s)
points(xvals, dpois(xvals, lambda_sum), col="red", pch=20)
...
# 1)
p_deg_25 <- 1 - ppois(cap1, lambda_site)
# 2)
p_deg_50 <- 1 - ppois(cap2, lambda_site)
# 3)
cap_1pct <- qpois(0.99, lambda_site)

# 4) Simular 60 minutos (1 hora)
mins <- rpois(60, lambda_site)
perc_deg_25 <- mean(mins > cap1) * 100
perc_deg_50 <- mean(mins > cap2) * 100

p_deg_25; p_deg_50; cap_1pct; perc_deg_25; perc_deg_50
...
```{r}
lambda_sales <- 8
maxI <- 25  # rango de búsqueda
costs <- numeric(maxI + 1)

for (I in 0:maxI) {
  # prob mass
  pmf <- dpois(0:100, lambda_sales)  # truncar en 100 por practicidad
  svals <- 0:100
  # adjust pmf tail beyond 100
  pmf[101] <- 1 - ppois(100, lambda_sales)  # remainder aggregated at 100 (aprox 0)
  
  # Unsold: sum_{s=0}^I (I - s) P(S=s)
  unsold <- sum( (I - svals[svals <= I]) * pmf[svals <= I] )
  # Lost: sum_{s=I+1}^{100} (s - I) P(S=s)
  lost <- sum( (svals[svals > I] - I) * pmf[svals > I] )
  
  costs[I + 1] <- 2 * unsold + 15 * lost + 10 * I
}

I_vals <- 0:maxI
bestI <- I_vals[which.min(costs)]
cat("I* =", bestI, " costo esperado =", min(costs), "\n")
# Mostrar tabla
data.frame(I=I_vals, Cost=round(costs,2))[1: (maxI+1), ]
