...
bolas <- 1:8  # bolas numeradas del 1 al 8

eventos_par <- bolas[bolas %% 2 == 0]  # selecciona los números pares

# probabilidad
probabilidad_par <- length(eventos_par) / length(bolas)

# Resultados
cat("Bolas en la urna:", bolas, "\n")
cat("Números pares:", eventos_par, "\n")
...
muestras <- sample(valores, size = 500, replace = TRUE)


freq_obs <- table(muestras)


n <- 500
p_teorico <- 1 / length(valores)
freq_teorica <- rep(n * p_teorico, length(valores))
...
barplot(
  rbind(freq_obs, freq_teorica),
  beside = TRUE,
  names.arg = valores,
  col = c("skyblue", "orange"),
  legend.text = c("Observada", "Teórica"),
  main = "Comparación de frecuencias observadas vs teóricas",
  ylab = "Frecuencia"
)
