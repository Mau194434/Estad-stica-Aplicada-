...
# Parámetros
dfs <- 5           # grados de libertad
sample_sizes <- c(100, 300, 1000)

# Valores teóricos
media_teorica <- 0
# Varianza teórica de t(df) solo si df > 2
...
# --------------------------------------------------
# Histograma de la muestra más grande (n=1000)
# --------------------------------------------------
...
ggplot(df_hist, aes(x = x)) +
  ...
  # Densidad teórica t(df=5)
  stat_function(fun = function(x) dt(x, df = dfs), color = "red", size = 1.2) +
  # Densidad de normal estándar
...
# Histograma y densidad teórica t(df)
...
X <- matrix(rnorm(n * num_muestras), nrow = num_muestras, ncol = n)

# Calcular media y desviación estándar muestral de cada muestra
medias <- rowMeans(X)
desv <- apply(X, 1, sd)

# Calcular variable T
T <- medias / (desv / sqrt(n))

# --------------------------------------------------
# Histograma de T con densidad teórica t(df)
# --------------------------------------------------
x_vals <- seq(min(T)-1, max(T)+1, length.out = 200)
dens_teorica <- dt(x_vals, df = df)
df_hist <- data.frame(T = T)
...
ggplot(df_hist, aes(x = T)) +
  geom_histogram(aes(y = ..density..), bins = 40,
                 fill = "skyblue", color = "black", alpha = 0.7) +
  geom_line(data = df_dens, aes(x = x, y = dens), color = "red", size = 1.2) +
  labs(title = paste("Histograma de T (muestras n=", n, ") y densidad teórica t(", df, ")", sep=""),
       x = "Valores de T", y = "Densidad",
       subtitle = "Rojo: densidad teórica t(9)") +
  theme_minimal()

cat("Media simulada de T:", round(mean(T),3), "\n")
cat("Varianza simulada de T:", round(var(T),3), "\n")
cat("Media teórica t(9): 0, Varianza teórica t(9):", df/(df-2), "\n")
