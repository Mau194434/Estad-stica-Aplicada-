...
x_vals <- seq(0, max(F1), length.out = 200)
dens_teorica1 <- df(x_vals, df1, df2)
df_hist1 <- data.frame(F1 = F1)
df_dens1 <- data.frame(x = x_vals, dens = dens_teorica1)

p1 <- ggplot(df_hist1, aes(x = F1)) +
  geom_histogram(aes(y = ..density..), bins = 50,
                 fill = "skyblue", alpha = 0.6, color = "black") +
  geom_line(data = df_dens1, aes(x = x, y = dens), color = "red", size = 1.2) +
  labs(title = paste("X ~ t(", df_t, ") => X^2 ~ F(1,", df_t, ")", sep=""),
...
a <- 5
b <- 10
X2 <- rf(n_sim, df1 = a, df2 = b)
F2 <- 1 / X2
df_vals2 <- data.frame(x = seq(0, max(F2), length.out = 200),
                       dens = df(seq(0, max(F2), length.out = 200), df1 = b, df2 = a))
df_hist2 <- data.frame(F2 = F2)

p2 <- ggplot(df_hist2, aes(x = F2)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "skyblue", alpha = 0.6, color = "black") +
  geom_line(data = df_vals2, aes(x = x, y = dens), color = "red", size = 1.2) +
  labs(title = paste("X ~ F(", a,",",b,") => 1/X ~ F(", b,",",a,")", sep=""),
       x = "Valores", y = "Densidad") +
  theme_minimal()

# -----------------------------------------------------------------
# 3) X ~ chi2(a), Y ~ chi2(b) independientes => (X/a)/(Y/b) ~ F(a,b)
# -----------------------------------------------------------------
a_chi <- 5
b_chi <- 10
X3 <- rchisq(n_sim, df = a_chi)
Y3 <- rchisq(n_sim, df = b_chi)
F3 <- (X3 / a_chi) / (Y3 / b_chi)
x_vals3 <- seq(0, max(F3), length.out = 200)
dens_teorica3 <- df(x_vals3, df1 = a_chi, df2 = b_chi)
df_hist3 <- data.frame(F3 = F3)
df_dens3 <- data.frame(x = x_vals3, dens = dens_teorica3)

p3 <- ggplot(df_hist3, aes(x = F3)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "skyblue", alpha = 0.6, color = "black") +
  geom_line(data = df_dens3, aes(x = x, y = dens), color = "red", size = 1.2) +
  labs(title = paste("X~χ²(",a_chi,"), Y~χ²(",b_chi,") => (X/a)/(Y/b) ~ F(",a_chi,",",b_chi,")", sep=""),
       x = "Valores", y = "Densidad") +
  theme_minimal()
