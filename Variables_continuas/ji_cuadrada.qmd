...
dfs <- c(5, 20, 50)                   # grados de libertad
sample_sizes <- c(100, 500, 1000, 2000, 5000, 10000)

# Iteramos sobre cada valor de n (grados de libertad)
for (df in dfs) {
  ...
  p1 <- ggplot(df_media, aes(x = Tamaño, y = Media)) +
    ...
    geom_hline(yintercept = mean_theoretical, linetype = "dashed", color = "red") +
    scale_x_log10() +
  ...
  p3 <- ggplot(df_hist, aes(x = x)) +
    geom_histogram(aes(y = ..density..), bins = 50, fill = "skyblue", alpha = 0.7, color = "black") +
...
sample_sizes <- c(2, 5, 10, 30, 50, 100)  # tamaños de muestra a comparar
...
for (n in sample_sizes) {
  ...
  # Estandarizar las medias muestrales
...
ggplot(df_tcl, aes(x = Z, fill = Tamaño)) +
  geom_histogram(aes(y = ..density..), bins = 50, alpha = 0.6, color = "black") +
...
x_vals <- seq(0, max(X_sim), length.out = 200)
...
ggplot(data.frame(X_sim), aes(x = X_sim)) +
  ...
  geom_line(data = dens_df, aes(x = x, y = dens), color = "red", size = 1.2) +
...
dfs <- c(10, 30, 50, 100)
n_sim <- 10000  # tamaño de cada muestra
...
# Simular dos variables ji-cuadrada independientes
